...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.advanceSetting.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.advance-settings.update", [$advanceSetting->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\AdvanceSetting::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $advanceSetting->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('key') ? 'is-invalid' : '' }}" type="text" name="key" id="key" value="{{ old('key', $advanceSetting->key) }}">
                @if($errors->has('key'))
                    <div class="invalid-feedback">
                        {{ $errors->first('key') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text" name="status" id="status" value="{{ old('status', $advanceSetting->status) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('project_code') ? 'is-invalid' : '' }}" type="text" name="project_code" id="project_code" value="{{ old('project_code', $advanceSetting->project_code) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $advanceSetting->amount) }}" step="1">
