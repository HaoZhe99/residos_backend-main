    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.rents.update", [$rent->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tenant') ? 'is-invalid' : '' }}" name="tenant_id" id="tenant_id" required>
                    @foreach($tenants as $id => $tenant)
                        <option value="{{ $id }}" {{ (old('tenant_id') ? old('tenant_id') : $rent->tenant->id ?? '') == $id ? 'selected' : '' }}>{{ $tenant }}</option>
                    @endforeach
                </select>
            ...
            {{-- <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tenant') ? 'is-invalid' : '' }}" type="text" name="tenant" id="tenant" value="{{ old('tenant', $rent->tenant) }}" required>
                @if($errors->has('tenant'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tenant') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rent_fee') ? 'is-invalid' : '' }}" type="number" name="rent_fee" id="rent_fee" value="{{ old('rent_fee', $rent->rent_fee) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('day_of_month') ? 'is-invalid' : '' }}" name="day_of_month" id="day_of_month" required>
                    ...
                    @foreach(App\Models\Rent::DAY_OF_MONTH_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('day_of_month', $rent->day_of_month) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('leases') ? 'is-invalid' : '' }}" type="number" name="leases" id="leases" value="{{ old('leases', $rent->leases) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_rent_day') ? 'is-invalid' : '' }}" type="text" name="start_rent_day" id="start_rent_day" value="{{ old('start_rent_day', $rent->start_rent_day) }}">
            ...
            {{-- <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_rent_day') ? 'is-invalid' : '' }}" type="text" name="end_rent_day" id="end_rent_day" value="{{ old('end_rent_day', $rent->end_rent_day) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bank_acc') ? 'is-invalid' : '' }}" type="text" name="bank_acc" id="bank_acc" value="{{ old('bank_acc', $rent->bank_acc) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    <option value disabled {{ old('ststus', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Rent::STSTUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $rent->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Rent::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $rent->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slot_limit') ? 'is-invalid' : '' }}" type="number" name="slot_limit" id="slot_limit" value="{{ old('slot_limit', $rent->slot_limit) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('room_size') ? 'is-invalid' : '' }}" type="number" name="room_size" id="room_size" value="{{ old('room_size', $rent->room_size) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('remark') ? 'is-invalid' : '' }}" type="text" name="remark" id="remark" value="{{ old('remark', $rent->remark) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('amenities') ? 'is-invalid' : '' }}" name="amenities[]" id="amenities" multiple>
                    @foreach($amenities as $id => $amenities)
                        <option value="{{ $id }}" {{ (in_array($id, old('amenities', [])) || $rent->amenities->contains($id)) ? 'selected' : '' }}>{{ $amenities }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('unit_owner') ? 'is-invalid' : '' }}" name="unit_owner_id" id="unit_owner_id" required>
                    @foreach($unit_owners as $id => $unit_owner)
                        <option value="{{ $id }}" {{ (old('unit_owner_id') ? old('unit_owner_id') : $rent->unit_owner->id ?? '') == $id ? 'selected' : '' }}>{{ $unit_owner }}</option>
                    @endforeach
                </select>
