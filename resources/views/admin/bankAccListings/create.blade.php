...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.bankAccListing.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.bank-acc-listings.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="bank_account">{{ trans('cruds.bankAccListing.fields.bank_account') }}</label>
                <input class="form-control {{ $errors->has('bank_account') ? 'is-invalid' : '' }}" type="text" name="bank_account" id="bank_account" value="{{ old('bank_account', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bank_name') ? 'is-invalid' : '' }}" name="bank_name_id" id="bank_name_id">
                    @foreach($bank_names as $id => $bank_name)
                        <option value="{{ $id }}" {{ old('bank_name_id') == $id ? 'selected' : '' }}>{{ $bank_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="project_code_id">{{ trans('cruds.bankAccListing.fields.project_code') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('project_code_id') ? 'is-invalid' : '' }}" name="project_code_id" id="project_code_id" multiple>
