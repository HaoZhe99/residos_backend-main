...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.projectListing.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.project-listings.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('developer') ? 'is-invalid' : '' }}" name="developer_id" id="developer_id" required>
                    @foreach($developers as $id => $developer)
                        <option value="{{ $id }}" {{ old('developer_id') == $id ? 'selected' : '' }}>{{ $developer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tenure') ? 'is-invalid' : '' }}" name="tenure" id="tenure" required>
                    ...
                    @foreach(App\Models\ProjectListing::TENURE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tenure', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime{{ $errors->has('completion_date') ? 'is-invalid' : '' }}" type="text" name="completion_date" id="completion_date" value="{{ old('completion_date', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sales_gallery') ? 'is-invalid' : '' }}" type="text" name="sales_gallery" id="sales_gallery" value="{{ old('sales_gallery', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('block') ? 'is-invalid' : '' }}" type="number" name="block" id="block" value="{{ old('block', '') }}" step="1">
                @if($errors->has('block'))
                    <div class="invalid-feedback">
                        {{ $errors->first('block') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pic') ? 'is-invalid' : '' }}" name="pic_id" id="pic_id" required>
                    @foreach($pics as $id => $pic)
                        <option value="{{ $id }}" {{ old('pic_id') == $id ? 'selected' : '' }}>{{ $pic }}</option>
                    @endforeach
                </select>
                @if($errors->has('pic'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <label for="role_id">{{ trans('cruds.projectListing.fields.create_by') }}</label>
                <select class="form-control select2 {{ $errors->has('create_by') ? 'is-invalid' : '' }}" name="create_by" id="create_by" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ old('create_by') ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
