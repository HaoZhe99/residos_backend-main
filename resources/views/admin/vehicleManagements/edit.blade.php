    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.vehicle-managements.update", [$vehicleManagement->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('driver') ? 'is-invalid' : '' }}" type="text" name="driver" id="driver" value="{{ old('driver', $vehicleManagement->driver) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $vehicleManagement->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('carpark_location') ? 'is-invalid' : '' }}" name="carpark_location_id" id="carpark_location_id">
                    @foreach($carpark_locations as $id => $carpark_location)
                        <option value="{{ $id }}" {{ (old('carpark_location_id') ? old('carpark_location_id') : $vehicleManagement->carpark_location->id ?? '') == $id ? 'selected' : '' }}>{{ $carpark_location }}</option>
                    @endforeach
                </select>
