...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.facilityListing.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.facility-listings.update", [$facilityListing->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="facility_code">{{ trans('cruds.facilityListing.fields.facility_code') }}</label>
                <input class="form-control {{ $errors->has('facility_code') ? 'is-invalid' : '' }}" type="text" name="facility_code" id="facility_code" value="{{ old('facility_code', $facilityListing->facility_code) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $facilityListing->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('desctiption') ? 'is-invalid' : '' }}" name="desctiption" id="desctiption">{!! old('desctiption', $facilityListing->desctiption) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\FacilityListing::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $facilityListing->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('open') ? 'is-invalid' : '' }}" type="text" name="open" id="open" value="{{ old('open', $facilityListing->open) }}" required>
                @if($errors->has('open'))
                    <div class="invalid-feedback">
                        {{ $errors->first('open') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('closed') ? 'is-invalid' : '' }}" type="text" name="closed" id="closed" value="{{ old('closed', $facilityListing->closed) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    @foreach($categories as $id => $category)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $facilityListing->category->id ?? '') == $id ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('project_code') ? 'is-invalid' : '' }}" name="project_code_id" id="project_code_id" required>
                    @foreach($project_codes as $id => $project_code)
                        <option value="{{ $id }}" {{ (old('project_code_id') ? old('project_code_id') : $facilityListing->project_code->id ?? '') == $id ? 'selected' : '' }}>{{ $project_code }}</option>
                    @endforeach
                </select>
