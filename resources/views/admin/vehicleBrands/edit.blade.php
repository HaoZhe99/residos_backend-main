        ...
        <form method="POST" action="{{ route("admin.vehicle-brands.update", [$vehicleBrand->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_enable') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_enable" id="is_enable" value="1" {{ $vehicleBrand->is_enable || old('is_enable', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_enable">{{ trans('cruds.vehicleBrand.fields.is_enable') }}</label>
                </div>
