...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.gateHistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.gate-histories.update", [$gateHistory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="gate_code">{{ trans('cruds.gateHistory.fields.gate_code') }}</label>
                <input class="form-control {{ $errors->has('gate_code') ? 'is-invalid' : '' }}" type="text" name="gate_code" id="gate_code" value="{{ old('gate_code', $gateHistory->gate_code) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $gateHistory->type) }}" required>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
                        {{ $errors->first('type') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('username') ? 'is-invalid' : '' }}" name="username_id" id="username_id" required>
                    @foreach($usernames as $id => $username)
                        <option value="{{ $id }}" {{ (old('username_id') ? old('username_id') : $gateHistory->username->id ?? '') == $id ? 'selected' : '' }}>{{ $username }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gateway') ? 'is-invalid' : '' }}" name="gateway_id" id="gateway_id" required>
                    @foreach($gateways as $id => $gateway)
                        <option value="{{ $id }}" {{ (old('gateway_id') ? old('gateway_id') : $history->gateway->id ?? '') == $id ? 'selected' : '' }}>{{ $gateway }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('qr') ? 'is-invalid' : '' }}" name="qr_id" id="qr_id">
                    @foreach($qrs as $id => $qr)
                        <option value="{{ $id }}" {{ (old('qr_id') ? old('qr_id') : $history->qr->id ?? '') == $id ? 'selected' : '' }}>{{ $qr }}</option>
                    @endforeach
                </select>
                @if($errors->has('qr'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('unit') ? 'is-invalid' : '' }}" name="unit_id" id="unit_id">
                    @foreach($units as $id => $unit)
                        <option value="{{ $id }}" {{ (old('unit_id') ? old('unit_id') : $history->unit->id ?? '') == $id ? 'selected' : '' }}>{{ $unit }}</option>
                    @endforeach
                </select>
                @if($errors->has('unit'))
                    <div class="invalid-feedback">
