...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} Water Utility Setting
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.water-utility-settings.update", [$waterUtilitySetting->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="amount_per_consumption">amount_per_consumption</label>
                <input class="form-control {{ $errors->has('amount_per_consumption') ? 'is-invalid' : '' }}" type="number" name="amount_per_consumption" id="amount_per_consumption" value="{{ old('amount_per_consumption', $waterUtilitySetting->amount_per_consumption) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('project_code') ? 'is-invalid' : '' }}" name="project_code_id" id="project_code_id" required>
                    @foreach($project_code as $id => $project_code)
                        <option value="{{ $id }}" {{ (old('project_code_id') ? old('project_code_id') : $waterUtilityPayment->project_code->id ?? '') == $id ? 'selected' : '' }}>{{ $project_code }}</option>
                    @endforeach
                </select>
