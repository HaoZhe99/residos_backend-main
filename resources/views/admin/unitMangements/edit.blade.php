...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.unitMangement.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.unit-mangements.update", [$unitMangement->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="unit_code">{{ trans('cruds.unitMangement.fields.unit_code') }}</label>
                <input class="form-control {{ $errors->has('unit_code') ? 'is-invalid' : '' }}" type="text" name="unit_code" id="unit_code" value="{{ old('unit_code', $unitMangement->unit_code) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('floor_size') ? 'is-invalid' : '' }}" type="text" name="floor_size" id="floor_size" value="{{ old('floor_size', $unitMangement->floor_size) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bed_room') ? 'is-invalid' : '' }}" type="text" name="bed_room" id="bed_room" value="{{ old('bed_room', $unitMangement->bed_room) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('toilet') ? 'is-invalid' : '' }}" type="text" name="toilet" id="toilet" value="{{ old('toilet', $unitMangement->toilet) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('floor_level') ? 'is-invalid' : '' }}" type="text" name="floor_level" id="floor_level" value="{{ old('floor_level', $unitMangement->floor_level) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('carpark_slot') ? 'is-invalid' : '' }}" type="text" name="carpark_slot" id="carpark_slot" value="{{ old('carpark_slot', $unitMangement->carpark_slot) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('bumi_lot') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="bumi_lot" id="bumi_lot" value="1" {{ $unitMangement->bumi_lot || old('bumi_lot', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('block') ? 'is-invalid' : '' }}" type="text" name="block" id="block" value="{{ old('block', $unitMangement->block) }}">
                @if($errors->has('block'))
                    <div class="invalid-feedback">
                        {{ $errors->first('block') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\UnitManagement::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $unitMangement->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('balcony') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="balcony" id="balcony" value="1" {{ $unitMangement->balcony || old('balcony', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('unit') ? 'is-invalid' : '' }}" name="unit_id" id="unit_id" required>
                    @foreach($units as $id => $unit)
                        <option value="{{ $id }}" {{ (old('unit_id') ? old('unit_id') : $unitMangement->unit->id ?? '') == $id ? 'selected' : '' }}>{{ $unit }}</option>
                    @endforeach
                </select>
                @if($errors->has('unit'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('owner') ? 'is-invalid' : '' }}" name="owner_id" id="owner_id">
                    @foreach($owners as $id => $owner)
                        <option value="{{ $id }}" {{ (old('owner_id') ? old('owner_id') : $unitMangement->owner->id ?? '') == $id ? 'selected' : '' }}>{{ $owner }}</option>
                    @endforeach
                </select>
                @if($errors->has('owner'))
                    <div class="invalid-feedback">
