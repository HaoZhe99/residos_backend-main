    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.gateways.update", [$gateway->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('last_active') ? 'is-invalid' : '' }}" type="text" name="last_active" id="last_active" value="{{ old('last_active', $gateway->last_active) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('in_enable') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="in_enable" id="in_enable" value="1" {{ $gateway->in_enable || old('in_enable', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('guard') ? 'is-invalid' : '' }}" type="text" name="guard" id="guard" value="{{ old('guard', $gateway->guard) }}" required>
                @if($errors->has('guard'))
                    <div class="invalid-feedback">
                        {{ $errors->first('guard') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('location_code') ? 'is-invalid' : '' }}" name="location_code_id" id="location_code_id">
                    @foreach($location_codes as $id => $location_code)
                        <option value="{{ $id }}" {{ (old('location_code_id') ? old('location_code_id') : $gateway->location_code->id ?? '') == $id ? 'selected' : '' }}>{{ $location_code }}</option>
                    @endforeach
                </select>
