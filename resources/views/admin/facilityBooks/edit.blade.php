    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.facility-books.update", [$facilityBook->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text" name="status" id="status" value="{{ old('status', $facilityBook->status) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('username') ? 'is-invalid' : '' }}" name="username_id" id="username_id" required>
                    @foreach($usernames as $id => $username)
                        <option value="{{ $id }}" {{ (old('username_id') ? old('username_id') : $facilityBook->username->id ?? '') == $id ? 'selected' : '' }}>{{ $username }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('project_code') ? 'is-invalid' : '' }}" name="project_code_id" id="project_code_id" required>
                    @foreach($project_codes as $id => $project_code)
                        <option value="{{ $id }}" {{ (old('project_code_id') ? old('project_code_id') : $facilityBook->project_code->id ?? '') == $id ? 'selected' : '' }}>{{ $project_code }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('facility_code') ? 'is-invalid' : '' }}" name="facility_code_id" id="facility_code_id" required>
                    @foreach($facility_codes as $id => $facility_code)
                        <option value="{{ $id }}" {{ (old('facility_code_id') ? old('facility_code_id') : $facilityBook->facility_code->id ?? '') == $id ? 'selected' : '' }}>{{ $facility_code }}</option>
                    @endforeach
                </select>
