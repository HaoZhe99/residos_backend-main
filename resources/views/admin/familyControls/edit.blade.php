...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.family-controls.update", [$familyControl->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('family') ? 'is-invalid' : '' }}" name="family_id" id="family_id" required>
                    @foreach($families as $id => $family)
                        <option value="{{ $id }}" {{ (old('family_id') ? old('family_id') : $familyControl->family->id ?? '') == $id ? 'selected' : '' }}>{{ $family }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('unit_owner') ? 'is-invalid' : '' }}" name="unit_owner_id" id="unit_owner_id" required>
                    @foreach($unit_owners as $id => $unit_owner)
                        <option value="{{ $id }}" {{ (old('unit_owner_id') ? old('unit_owner_id') : $familyControl->unit_owner->id ?? '') == $id ? 'selected' : '' }}>{{ $unit_owner }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('activity_logs') ? 'is-invalid' : '' }}" name="activity_logs" id="activity_logs" required>
                    ...
                    @foreach(App\Models\FamilyControl::ACTIVITY_LOGS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('activity_logs', $familyControl->activity_logs) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('from_family') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="from_family" id="from_family" value="1" {{ $familyControl->from_family || old('from_family', 0) === 1 ? 'checked' : '' }}>
