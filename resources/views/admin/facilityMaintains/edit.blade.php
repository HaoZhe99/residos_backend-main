...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.facilityMaintain.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.facility-maintains.update", [$facilityMaintain->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="remark">{{ trans('cruds.facilityMaintain.fields.remark') }}</label>
                <textarea class="form-control {{ $errors->has('remark') ? 'is-invalid' : '' }}" name="remark" id="remark">{{ old('remark', $facilityMaintain->remark) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('username') ? 'is-invalid' : '' }}" name="username_id" id="username_id" required>
                    @foreach($usernames as $id => $username)
                        <option value="{{ $id }}" {{ (old('username_id') ? old('username_id') : $facilityMaintain->username->id ?? '') == $id ? 'selected' : '' }}>{{ $username }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('facility_code') ? 'is-invalid' : '' }}" name="facility_code_id" id="facility_code_id" required>
                    @foreach($facility_codes as $id => $facility_code)
                        <option value="{{ $id }}" {{ (old('facility_code_id') ? old('facility_code_id') : $facilityMaintain->facility_code->id ?? '') == $id ? 'selected' : '' }}>{{ $facility_code }}</option>
                    @endforeach
                </select>
